...
export async function fetchSocialLinksBatch(uids, placeIdMap = {}) {
  const all = {};
  for (let i = 0; i < uids.length; i += 10) {
    try {
      const batch = uids.slice(i, i + 10);
      const pids = batch.map(uid => placeIdMap[uid] || "");
      const d = await api({ action: "social-links-batch", ids: batch.join(","), placeIds: pids.join(",") });
      Object.assign(all, d.results || {});
