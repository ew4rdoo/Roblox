...
export default function Dashboard() {
  ...
  const enrichGames = useCallback(async (universeIds) => {
    ...
    for (const batch of batches) {
      ...
      try {
        [details, votes, thumbs, icons, gp] = await Promise.all([
        ...
        if (abortRef.current) return all;
        // Fetch social links with placeId map (needed for page scraping fallback)
        const placeIdMap = {};
        details.forEach(d => { if (d.id && d.rootPlaceId) placeIdMap[d.id] = d.rootPlaceId; });
        social = await fetchSocialLinksBatch(batch, placeIdMap).catch(()=>({}));
        if (abortRef.current) return all;
      } catch {
  ...
  const runScan = useCallback(async () => {
    ...
    const totalSteps = sorts.length + allSearchTerms.length;
    ...
    log(`Hunting ${minPlayers}–${maxPlayers} CCU games across ${sorts.length} charts + ${allSearchTerms.length} searches (${PAGES_PER_SOURCE} pages each)${browseAll ? " + BROWSE ALL" : ""}...`);
    ...
    // ─── Sweep all search terms (keywords + browse) ─────
    for (let i = 0; i < allSearchTerms.length; i++) {
      if (abortRef.current) break;
      const s = sorts.length + i + 1;
      setProgress(p => ({ ...p, step: s }));
      log(`[${s}/${totalSteps}] Search: "${allSearchTerms[i]}"...`);
      try {
        const ids = await fetchPagesFromSearch(allSearchTerms[i], seenIds);
        if (ids.length) {
          const enriched = await enrichGames(ids);
          allGames.push(...enriched);
          setProgress(p => ({ ...p, found: allGames.length }));
    ...
    if (abortRef.current) {
      log(`⛔ Stopped early — kept ${final.length} games found so far.`);
    } else {
