...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Load dismissed list
      try {
        const r = await fetch(`/api/db?action=dismissed-load&device_id=${id}`);
        const d = await r.json();
        if (d.ids?.length) setDismissed(new Set(d.ids));
      } catch {
        try { setDismissed(new Set(JSON.parse(localStorage.getItem("gs_dismissed") || "[]"))); } catch {}
      }
    })();
  }, []);
  ...
  const dismissGame = useCallback((uid) => {
    setDismissed(prev => {
      const next = new Set(prev);
      next.add(uid);
      try { localStorage.setItem("gs_dismissed", JSON.stringify([...next])); } catch {}
      const did = deviceIdRef.current;
      if (did) {
        fetch("/api/db", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ action: "dismissed-add", device_id: did, universe_id: uid }),
        }).catch(() => {});
      }
      return next;
    });
  ...
  const filtered = useMemo(() => {
    let g = [...games];
    // Hide dismissed games
    if (dismissed.size > 0) g = g.filter(x => !dismissed.has(x.universe_id));
    ...
    return g;
  }, [games, sortBy, filterTier, filterOutreach, filterPlateau, filterTrend, outreachStatuses, minRating, minFavVisitPct, maxDaysSinceUpdate, maxGameAge, requireGamepasses, dismissed]);
        ...
        <>
          ...
          {!searchLoading && searchResults.length > 0 && (
            <>
              ...
              {searchResults.filter(g => !dismissed.has(g.universe_id)).map((g, i) => (
      ...
      {activeTab === "scanner" && (
        ...
        {games.length > 0 && <section style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 18, marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            {dismissed.size > 0 && <button onClick={()=>{ setDismissed(new Set()); try { localStorage.setItem("gs_dismissed","[]"); } catch {} }} className="mono" style={{ ...chip(false), color: "rgba(239,68,68,0.5)", borderColor: "rgba(239,68,68,0.15)" }}>↩ Restore {dismissed.size} dismissed</button>}
          </div>
        </section>}
