...
// Broad single-char / common-word searches that match nearly everything on Roblox
...
const PAGES_PER_SOURCE = 10; // Fetch 10 pages per chart/search
...
export default function Dashboard() {
  ...
  const runScan = useCallback(async () => {
    ...
    const browseTerms = browseAll ? BROWSE_ALL_TERMS : [];
    const allSearchTerms = [...keywords, ...browseTerms.filter(t => !keywords.includes(t))];
    ...
    log(`⚡ PHASE 1: Collecting IDs from ${sorts.length} charts + ${allSearchTerms.length} searches (${PAGES_PER_SOURCE} pages each)${browseAll ? " + BROWSE ALL" : ""}...`);
    ...
    for (let i = 0; i < sorts.length; i += 3) {
      ...
      const results = await Promise.all(batch.map(async (sort, idx) => {
        try {
          return await fetchPagesFromSort(sort.sortId, sortsSessionId, seenIds, sort.name);
        } catch { return []; }
      }));
    ...
    // Run search fetches in parallel batches of 5
    for (let i = 0; i < allSearchTerms.length; i += 5) {
      if (abortRef.current) break;
      const batch = allSearchTerms.slice(i, i + 5);
      await Promise.all(batch.map(async (term) => {
        try {
          return await fetchPagesFromSearch(term, seenIds);
        } catch { return []; }
      }));
      const step = sorts.length + Math.min(i + 5, allSearchTerms.length);
      setProgress(p => ({ ...p, step, found: seenIds.size }));
      if ((i % 25) === 0 || i + 5 >= allSearchTerms.length) {
        log(`  Searches ${i+1}-${Math.min(i+5, allSearchTerms.length)} done · ${seenIds.size} unique IDs`);
      }
    ...
    const PARALLEL = 2; // 2 enrichment batches at once
    ...
    setScanning(false);
  }, [availableSorts, sortsSessionId, extraKeywords, browseAll, minPlayers, maxPlayers, enrichGames, fetchPagesFromSort, fetchPagesFromSearch, saveToDb, dbStatus, log]);
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px 24px" }}>
        ...
        <section style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 18, marginBottom: 16 }}>
          <div className="mono" style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 14, lineHeight: 1.7 }}>
            Sweeps <b style={{ color: "rgba(255,255,255,0.5)" }}>{availableSorts.length || "..."} charts</b> + <b style={{ color: "rgba(255,255,255,0.5)" }}>{extraKeywords.length} keyword searches</b>{browseAll ? <> + <b style={{ color: "#f59e0b" }}>{BROWSE_ALL_TERMS.length} broad searches</b></> : ""} · {PAGES_PER_SOURCE} pages each · gamepasses + social links · {dbStatus === "connected" ? "auto-saves to DB" : "DB offline"}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto auto", gap: 10, alignItems: "end" }}>
            ...
            <div><label style={fl}>Browse All</label>
              <button onClick={()=>setBrowseAll(!browseAll)} className="mono" style={{ padding: "9px 14px", fontSize: 12, borderRadius: 6, cursor: "pointer", border: `1px solid ${browseAll ? "#f59e0b50" : "rgba(255,255,255,0.07)"}`, background: browseAll ? "rgba(245,158,11,0.15)" : "rgba(255,255,255,0.04)", color: browseAll ? "#f59e0b" : "rgba(255,255,255,0.3)", fontFamily: "'JetBrains Mono',monospace", whiteSpace: "nowrap" }}>{browseAll ? "✓ ON" : "OFF"}</button>
            </div>
            <div>{!scanning
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={() => setShowAdvanced(!showAdvanced)} className="mono" style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", background: "none", border: "none", cursor: "pointer", padding: 0, textDecoration: "underline", textUnderlineOffset: 3 }}>
              {showAdvanced ? "▾ Hide" : "▸ Show"} keyword searches ({extraKeywords.length} active{browseAll ? ` + ${BROWSE_ALL_TERMS.length} browse-all` : ""})
            </button>
        ...
        {!games.length && !scanning && dbLoaded && <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <div className="mono" style={{ fontSize: 12, color: "rgba(255,255,255,0.18)", maxWidth: 420, margin: "0 auto", lineHeight: 1.8 }}>
            All {SEARCH_PRESETS.length} genre keywords are pre-selected. Toggle <span style={{ color: "#f59e0b" }}>Browse All</span> to also search A-Z + common words for maximum coverage. Hit <span style={{ color: "#10b981" }}>SCAN</span> to sweep every source with {PAGES_PER_SOURCE} pages each.
          </div>
        </div>}
      </main>
