...
export default function Dashboard() {
  ...
  // Database state
  const [dbStatus, setDbStatus] = useState("checking"); // checking, connected, unavailable
  ...
  const log = useCallback((msg) => {
    setScanLog(p => [...p.slice(-80), { time: new Date().toLocaleTimeString(), msg }]);
      ...
      if (health?.ok) {
        ...
        if (result?.games?.length) {
          const loaded = result.games.map(g => ({
            ...
            score_breakdown: null, // DB doesn't store full breakdown
          }));
          setGames(loaded);
          // Load outreach statuses
          const statuses = {};
  ...
  const runScan = useCallback(async () => {
    ...
    const total = sorts.length + keywords.length;
    if (!total) { setError("No charts loaded."); setScanning(false); return; }
    ...
    setProgress({ step: 0, total, found: 0 });
    log(`Hunting ${minPlayers}–${maxPlayers} CCU games...`);

    for (let i = 0; i < sorts.length; i++) {
      if (abortRef.current) break;
      setProgress(p => ({ ...p, step: i + 1 }));
      log(`[${i+1}/${total}] ${sorts[i].name}...`);
      try {
        const data = await getSortContent(sorts[i].sortId, sortsSessionId);
        ...
        if (ids.length) {
          ...
          log(enriched.length ? `  ✓ ${enriched.length} in range` : `  — 0 in range`);
    ...
    for (let i = 0; i < keywords.length; i++) {
      ...
      log(`[${s}/${total}] Search: "${keywords[i]}"...`);
      try {
        const data = await searchGames(keywords[i], Math.random().toString(36).slice(2));
        const ids = (data._extractedUniverseIds||[]).filter(id => !seenIds.has(id));
        ids.forEach(id => seenIds.add(id));
        if (ids.length) {
          const enriched = await enrichGames(ids);
    ...
    setGames(final);
    log(`Scan complete! ${final.length} games found.`);
    ...
    setScanning(false);
  }, [availableSorts, sortsSessionId, extraKeywords, minPlayers, maxPlayers, enrichGames, saveToDb, dbStatus, log]);
  ...
  const tierCounts = useMemo(() => {
    ...
    games.forEach(g => c[getTier(g.potential_score || g.total_score)]++);
    return c;
  }, [games]);
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px 24px" }}>
        {/* ─── DB loaded previous results notice ──────────── */}
        {dbLoaded && games.length > 0 && !scanning && scanLog.length === 0 && (
          <div className="mono" style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.1)", borderRadius: 8, padding: "10px 14px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ color: "rgba(255,255,255,0.15)" }}>Run a new scan to refresh data</span>
          </div>
        )}
        ...
        <section style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 18, marginBottom: 16 }}>
          <div className="mono" style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginBottom: 14, lineHeight: 1.7 }}>
            Sweeps all <b style={{ color: "rgba(255,255,255,0.5)" }}>{availableSorts.length || "..."} charts</b> · fetches gamepasses + social links · {dbStatus === "connected" ? "auto-saves to database for trend tracking" : "database offline — results stored locally"}
          </div>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={() => setShowAdvanced(!showAdvanced)} className="mono" style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", background: "none", border: "none", cursor: "pointer", padding: 0, textDecoration: "underline", textUnderlineOffset: 3 }}>
              {showAdvanced ? "▾ Hide" : "▸ Show"} keyword searches
            </button>
            {showAdvanced && <div style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.04)" }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <input type="text" value={kwInput} onChange={e=>setKwInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addKeyword()} placeholder="Custom..." style={{ ...inp, flex: 1, fontSize: 11, padding: "6px 10px" }} />
          ...
          {(scanning || scanLog.length > 0) && <div style={{ marginTop: 14 }}>
            ...
            <div ref={logRef} className="mono" style={{ maxHeight: 130, overflow: "auto", background: "rgba(0,0,0,0.25)", borderRadius: 6, padding: "8px 10px", fontSize: 10, color: "rgba(255,255,255,0.35)", lineHeight: 1.8 }}>
        ...
        {games.length > 0 && <section style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 10, padding: 18, marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            <button onClick={()=>{ setMinRating(85); setMinFavVisitPct(5); setMaxDaysSinceUpdate(14); setRequireGamepasses(true); setFilterOutreach("all"); setFilterTrend("all"); setFilterPlateau(false); }} className="mono" style={chip(false)}>🎯 Growth Target</button>
            <button onClick={()=>{ setMinRating(80); setMinFavVisitPct(0); setMaxDaysSinceUpdate(9999); setRequireGamepasses(false); setFilterOutreach("all"); setFilterTrend("all"); setFilterPlateau(true); }} className="mono" style={chip(false)}>📊 Plateaus Only</button>
        ...
        {games.length > 0 && <>
          ...
          {!filtered.length && <div className="mono" style={{ textAlign: "center", padding: 40, color: "rgba(255,255,255,0.2)" }}>No games match filters.</div>}
        </>}
        ...
        {!games.length && !scanning && dbLoaded && <div style={{ textAlign: "center", padding: "60px 20px" }}>
          ...
          <div className="mono" style={{ fontSize: 12, color: "rgba(255,255,255,0.18)", maxWidth: 420, margin: "0 auto", lineHeight: 1.8 }}>
            Set your player range and hit <span style={{ color: "#10b981" }}>SCAN</span>.<br /><br />
            Results auto-save to database. Run daily scans to build trend data and detect plateaus — games with high quality but stalled growth.
          </div>
        </div>}
      </main>
